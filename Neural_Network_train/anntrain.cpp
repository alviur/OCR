...
{
    cv::Mat layerSizes(1,4,CV_32SC1);//alojara los datos
    layerSizes.at<int>(0)=trainData.cols;//cantidad de caracteristicas
    layerSizes.at<int>(1)=nlayers;//numero de capas ocultas
    layerSizes.at<int>(2)=nlayers;//numero de clases
    layerSizes.at<int>(3)=numClasses;//numero de clases

   ann.create(layerSizes,CvANN_MLP::SIGMOID_SYM,0.1,1);//inicializa la red
