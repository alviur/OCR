...
//abs(inicio-tan(angleROtH)*abs(Corregida.cols/2-segmentacionx[i]))
           ...
           for(int i=0;i<segmentacionx.size()-2;i++)
           {
               ...
               for(int k=0;k<ultima.rows;k++)
               {
                   if(imgROtY.at<uchar>(k,segmentacionx[i])==255)
                   {
                       ...
                       segmentaciony[i]=k;
                       if(k>=Corregida.rows-2)segmentaciony[i]=Corregida.rows-20;
                       flag=true;
                   }
           ...
           for(int i=0;i<segmentacionx.size()-2;i++)
           {
               ...
               //qDebug()<<segmentaciony[i]<<" "<<Corregida.rows<<" "<<ultima.rows;
              // qDebug()<<segmentacionx[i+1]<<" "<<ultima.cols;
               letras2[i]=ultima(Rect(abs(segmentacionx[i]),abs(segmentaciony[i]),segmentacionx[i+1]-segmentacionx[i],Corregida.rows+15));
...
{
    QString path="/home/lex/Cv/Images/PlacasData/";
...
void MainWindow::on_pushButton_3_clicked()//extraer
{
    ...
    QString  letra;
     ...
     QString pathImages;
      ...
      for(int iter=0;iter<=1762;iter++)//NUMERO DE LETRAS
              {
...
//                         }
                        ...
                        save_image(letras2[i],index2,".jpg");
